...
export async function getAdminOverview() {
    try {
      ...
      if (code === ErrorCode.RESOURCE_NOT_FOUND) {
        return {
          success: false,
          error: "Endpoint chưa được triển khai trên server",
        };
      }
  ...
  export async function exportDashboardData(type = "summary") {
    try {
      ...
      if (res.status === 200) {
        ...
        link.href = url;
        link.setAttribute("download", `dashboard_${type}_${Date.now()}.csv`);
        document.body.appendChild(link);
        link.click();
        link.remove();
        return { success: true };
      }
      return { success: false, error: "Không thể export dữ liệu" };
    } catch (err) {
      console.error("Export error:", err);
      return { success: false, error: "Lỗi khi export dữ liệu" };
    }
  }
