...
export async function deleteInstructorRequest(requestId) {
  try {
    const res = await api.delete(`/api/instructor-requests/${requestId}`);
    if (res.status === 200 || res.status === 204) {
      return { success: true };
    }
...
export async function getAdminInstructorRequests(status = null, page = 1, size = 10, sortDir = "desc") {
  try {
    ...
    const res = await api.get(`/api/instructor-requests/admin?${params.toString()}`);
    if (res.status === 200) {
      return { success: true, data: res.data?.data };
    }
...
export async function getAdminInstructorRequestDetails(requestId) {
  try {
    const res = await api.get(`/api/instructor-requests/admin/${requestId}`);
    if (res.status === 200) {
      return { success: true, data: res.data?.data };
    }
...
export async function reviewInstructorRequest(requestId, action, adminNotes) {
  try {
    const res = await api.patch(`/api/instructor-requests/admin/${requestId}/review`, {
      action,
