...
export async function getPublicInstructorOverview(userId) {
  ...
  try {
    const res = await api.get(`/api/public/instructors/${userId}`);
    if (res.status === 200) {
      return { success: true, data: res.data?.data };
    }
...
/**
 * Courses only: GET /api/public/instructors/{userId}/courses
 */
export async function getPublicInstructorCourses(userId, { page = 1, pageSize = 12, keyword, skills } = {}) {
  ...
  try {
    ...
    const res = await api.get(`/api/public/instructors/${userId}/courses?${params.toString()}`);
    if (res.status === 200) {
      return { success: true, data: res.data?.data };
    }
