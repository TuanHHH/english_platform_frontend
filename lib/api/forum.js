...
export async function forumListThreadPosts(threadId, params = {}) {
  const res = await api.get(`/api/public/forum/threads/${threadId}/posts`, {
    params,
  });
  return unwrapPaged(res);
}
...
export async function appReplyThread(threadId, payload, opts = {}) {
  const res = await api.post(
    `/api/app/forum/threads/${threadId}/posts`,
    payload
  );
  return unwrap(res);
}
...
export async function appReportPost(postId, { reason }) {
  const res = await api.post(`/api/app/forum/posts/${postId}/report`, {
    reason,
  });
  return unwrap(res);
}
...
export async function appReportThread(threadId, { reason }) {
  const res = await api.post(`/api/app/forum/threads/${threadId}/report`, {
    reason,
  });
  return unwrap(res);
}
...
export async function appForumMyThreads(params = {}) {
  const res = await api.get("/api/app/forum/me/threads", { params });
  return unwrapPaged(res);
}
...
export async function appReplyToPost(threadId, parentPostId, payload) {
  // Backend dùng cùng endpoint tạo post nhưng kèm parentId
  const res = await api.post(
    `/api/app/forum/threads/${threadId}/posts`,
...
export async function appUpdateThread(id, payload) {
  const res = await api.put(`/api/app/forum/threads/${id}`, payload);
  return unwrap(res);
}

export async function appDeleteOwnThread(id) {
  const res = await api.delete(`/api/app/forum/threads/${id}`);
  return res?.status === 204 ? true : unwrap(res);
}
