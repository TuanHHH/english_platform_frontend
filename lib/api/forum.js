...
export async function reportPost(postId, reason) {
  try {
    const res = await api.post(`/api/forum/posts/${postId}/report`, { reason });
    if (res.status === 201 || res.status === 200) {
      return { success: true, data: res.data?.data };
    }
...
export async function reportThread(threadId, reason) {
  try {
    const res = await api.post(`/api/forum/threads/${threadId}/report`, { reason });
    if (res.status === 201 || res.status === 200) {
      return { success: true, data: res.data?.data };
    }
...
export async function adminGetCategories() {
  try {
    const res = await api.get("/api/forum/categories/admin");
    if (res.status === 200) {
      return { success: true, data: res.data?.data || [] };
    }
    return { success: false, error: "Không thể lấy danh sách chuyên mục" };
...
export async function adminGetReports(params = {}) {
  try {
    const res = await api.get("/api/forum/reports/admin", { params });
    if (res.status === 200) {
      return { success: true, data: res.data?.data };
    }
